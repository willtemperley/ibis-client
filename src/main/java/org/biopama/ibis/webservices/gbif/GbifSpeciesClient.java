package org.biopama.ibis.webservices.gbif;
...
public class GbifSpeciesClient {
	...
	public List<GbifSpecies> getSuggestions(String search) {
		...
		Type listType = new TypeToken<ArrayList<GbifSpecies>>() {
		}.getType();
	...
	public static void main(String[] args) throws MalformedURLException,
		...
		// List<CommonName> x =
		// c.getVernacularNames("http://www.gbif.org/species/2221612");
		// for (CommonName commonName : x) {
		// System.out.println(commonName.getVernacularName());
		// }
		// System.err.println(x);

		GbifSpecies x = c.getSpecies("http://api.gbif.org/species/5231338");
	...
	public List<CommonName> getVernacularNames(String url) {
		...
		try {
			...
			VernacularNameQuery v = gson.fromJson(json,
					VernacularNameQuery.class);
			return v.getResults();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public GbifSpecies getSpeciesFromRedlist(String value) {
		...
		try {
			...
			if (sp == null) {
				Notification.show("Could not find species with that redlist id.");
				return null;
			}
			
			sp.setSourceId(value);
	...
	public GbifSpecies getSpecies(Long speciesKey) {
		String json;
		try {
			json = readURL(baseSpeciesURL + speciesKey);
			GbifSpecies sp = gson.fromJson(json, GbifSpecies.class);
			return sp;
		} catch (IOException e) {
			e.printStackTrace();
